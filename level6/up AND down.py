...
def arrange(strng):
    list_of_words = strng.split()  # split zwraca liste elementow podzielonych wedlug wzorca
    is_sorted = True
    iterator = 0
    if len(list_of_words) != 0:
        while is_sorted:
            if iterator % 2 == 0:
                if len(list_of_words[iterator]) <= len(list_of_words[iterator+1]):
                    iterator = iterator + 1
                else:
                    list_of_words[iterator], list_of_words[iterator+1] = list_of_words[iterator+1], list_of_words[iterator]
                    iterator = 0
            else:
                if len(list_of_words[iterator]) >= len(list_of_words[iterator+1]):
                    iterator = iterator + 1
                    ...
                    list_of_words[iterator], list_of_words[iterator+1] = list_of_words[iterator+1], list_of_words[iterator]
                    iterator = 0
            if iterator == (len(list_of_words) -1):
                is_sorted = False
        for index, elem in enumerate(list_of_words):
            if index % 2 == 1:
                list_of_words[index] = elem.upper()
            else:
